...
export const CompactOrderAccordionActions = memo(function CompactOrderAccordionActions({
  ...
  return (
    <div className="relative grid content-start gap-2 border-t border-slate-100 bg-white p-3 sm:grid-cols-2">
      ...
      {primaryAction ? (
        <button type="button" disabled={primaryAction.disabled} title={primaryAction.title ?? primaryAction.label} onClick={() => run(primaryAction)} className={cn("inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg border px-4 text-sm font-black disabled:cursor-not-allowed disabled:opacity-50", actionClass(primaryAction))}>
          {primaryAction.icon}
          {primaryAction.label}
        </button>
      ) : null}
      {secondaryActions.map((action) => (
        <button key={action.id} type="button" disabled={action.disabled} title={action.title ?? action.label} aria-label={action.title ?? action.label} onClick={() => run(action)} className={cn("inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg border px-3 text-sm font-black disabled:cursor-not-allowed disabled:opacity-50", actionClass(action))}>
          {action.icon}
      ...
      {moreActions.length ? (
        <div className="relative">
          <button ref={triggerRef} type="button" className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg px-3 text-sm font-black text-slate-600 hover:bg-slate-50" onClick={() => setOpen((value) => !value)} aria-label="More order actions" aria-haspopup="menu" aria-expanded={open} title="More actions">
          ...
          {open && mounted ? createPortal(
            mobile ? (
              <div className="fixed inset-0 bg-slate-950/30" style={{ zIndex: Z_INDEX.toast + 10 }} role="presentation">
                <div ref={menuRef} className="safe-bottom fixed inset-x-0 bottom-0 rounded-t-2xl border border-slate-200 bg-white p-3 shadow-2xl" role="menu" aria-label="More order actions">
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <button type="button" className="grid size-11 place-items-center rounded-full text-slate-500 hover:bg-slate-100" onClick={() => setOpen(false)} aria-label="Close actions">
                      <X className="size-4" />
                    </button>
                  </div>
              ...
              <div ref={menuRef} className="fixed max-h-[min(26rem,calc(100vh-1.5rem))] w-56 overflow-y-auto rounded-xl border border-slate-200 bg-white p-1 shadow-2xl" style={{ left: position.left, top: position.top, zIndex: Z_INDEX.toast + 10 }} role="menu" aria-label="More order actions">
                {moreActions.map((action) => (
                  <button key={action.id} type="button" disabled={action.disabled} role="menuitem" className={cn("flex min-h-11 w-full items-center gap-2 rounded-lg px-3 text-left text-xs font-black disabled:opacity-50", action.variant === "danger" ? "text-red-600 hover:bg-red-50" : "text-slate-700 hover:bg-slate-50")} onClick={() => run(action)}>
                    {action.icon}
                    {action.label}
                  </button>
                ))}
