...
import { delayCardClass } from "./OrderAccordion.utils";
...
export const CompactOrderAccordion = memo(function CompactOrderAccordion(props: CompactOrderAccordionProps) {
  const { id, delay, highlighted, className, isOpen, items, facts, timeline, notes, primaryAction, secondaryActions, moreActions } = props;

  return (
    <article
      className={cn(
        "relative overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition-shadow",
    ...
    >
      ...
      <AnimatePresence initial={false}>
        {isOpen ? (
          ...
          >
            <CompactOrderAccordionBody items={items} facts={facts} timeline={timeline} notes={notes} />
