...
export default function LoyaltyPage() {
  ...
  useEffect(() => {
    ...
    void fetch("/api/owner/analytics", { cache: "no-store", signal: controller.signal })
      .then(async (response) => {
        const payload = await response.json().catch(() => ({})) as { data?: Analytics; error?: string };
        if (!response.ok) throw new Error(payload.error || "Loyalty data could not be loaded.");
        setData(payload.data ?? { customers: [], loyalty: [] });
      })
      .catch((reason: unknown) => {
        if (reason instanceof DOMException && reason.name === "AbortError") return;
        setError(reason instanceof Error ? reason.message : "Loyalty data could not be loaded.");
      })
      .finally(() => {
  ...
  return <div className="space-y-6"><SectionHeader title="Loyalty and Retention" description="Firestore-backed balances, tiers, order counts, and customer lifetime value." action={<Button asChild variant="outline"><Link href="/owner/settings?tab=loyalty"><Gift className="size-4" />Loyalty Rules</Link></Button>} />{error ? <Card><CardContent className="p-5 text-sm font-semibold text-destructive">{error}</CardContent></Card> : null}<section className="dashboard-grid"><Metric icon={<Users className="size-5" />} title="Known customers" value={String(rows.length)} /><Metric icon={<Star className="size-5" />} title="Active points" value={String(points)} /><Metric icon={<Gift className="size-5" />} title="VIP / Gold" value={String(rows.filter((row) => ["VIP", "Gold"].includes(row.tier)).length)} /></section><AdvancedDataTable title={loading ? "Loading loyalty accounts" : "Canonical loyalty accounts"} columns={columns} rows={rows} exportFilename="loyalty.csv" /></div>;
}
