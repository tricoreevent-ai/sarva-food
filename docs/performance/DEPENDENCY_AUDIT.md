...
No package deletion is safe for `v1.0.0-rc5`. The Phase 2 dependency fix is route ownership: config-only Firebase checks use `src/firebase/config.ts`, auth/profile services load at effect/action time, and owner/menu production data services no longer inflate the public shell.

## RC5 Addendum

The current source scan still shows Mapbox and XLSX isolated to map/import-export surfaces, Firebase/Auth narrowed to auth/detail/diagnostic/operational paths, and no dependency duplicated across `dependencies` and `devDependencies`. Dependencies with low static usage are retained because they are reached through dynamic imports, scripts, provider-gated routes, or framework/runtime integration.
