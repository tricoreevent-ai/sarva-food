...
| Active Orders hosted multi-role QA pending | Medium | 🟡 Pending Manual | Repository validation passes `smoke:operational` 24/24, including payment independence, Ready Signal, multi-ticket dining, partial-payment bill-only merge, completed holding/history, and Kitchen item-first card contracts, but hosted Owner/Manager/Waiter/Cashier/Kitchen browser evidence is external. | Run the RC5 enterprise hosted action matrix after deployment and capture console/network/printer evidence. |
