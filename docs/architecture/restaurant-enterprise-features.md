...
This document records the production-readiness coverage added on top of the existing Food Gedi architecture. The implementation stays incremental: persisted mock state models the workflows now, while Firebase collections and Cloud Functions can replace the mock store later.
...
Owner creates post -> Admin approves -> Food Gedi social account publishes -> Customer clicks deep link -> Food page opens -> Offer auto applies -> Fast checkout.
