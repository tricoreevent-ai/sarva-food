...
9. History, billing, reports, and analytics derive from repository `orders`/`customerOrders` records after completion.
...
| Kitchen Operations | `/api/owner/kitchen` | `/api/owner/kitchen/stream` | `kitchenOrders` |
| Order History / Reports | `/api/owner/orders`, `/api/owner/analytics`, `/api/owner/reports` | Read on navigation/filter | `orders` / `customerOrders` |
